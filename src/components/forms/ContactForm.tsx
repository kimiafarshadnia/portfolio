    ...
    return (
        <form onSubmit={submitHandler} className="md:basis-2/4 w-full flex flex-col gap-6 pb-6">
            <div className="flex flex-col gap-2">
                ...
                <input type="text" name="name" id="name" onChange={changeHandler} className='bg-lightMode dark:bg-[#2B2B2B] px-5 py-1 w-full rounded-lg placeholder:text-sm placeholder:text-[#BDBDBD]' placeholder="Enter Your Name here" />
            </div>
            <div className="flex flex-col gap-2">
                <label htmlFor="email" className="font-semibold capitalize text-secondary dark:text-white">Your Email Address</label>
                <input type="email" name="email" id="email" onChange={changeHandler} className='bg-lightMode dark:bg-[#2B2B2B] px-5 py-1 w-full rounded-lg placeholder:text-sm placeholder:text-[#BDBDBD]' placeholder="Example: johnbrown@gmail.com" />
            </div>
            <div className="flex flex-col gap-2">
                <label htmlFor="subject" className="font-semibold capitalize text-secondary dark:text-white">What’s This about?</label>
                <input type="text" name="subject" id="subject" onChange={changeHandler} className='bg-lightMode dark:bg-[#2B2B2B] px-5 py-1 w-full rounded-lg placeholder:text-sm placeholder:text-[#BDBDBD]' placeholder="Enter a subject" />
            </div>
            <div className="flex flex-col gap-2">
